...
class User:
    profile_data = pathlib.Path(os.path.realpath(f'{ROOT}/profile/profile.json'))
    ...
    def trash(cls, *args):
        if '-c' in args:
            if Out.inp('are you sure >').lower() == 'y':
                os.mkdir(pyQMD_PATH["trash"]) if not os.path.exists(
                    pyQMD_PATH["trash"]) else shutil.rmtree(pyQMD_PATH["trash"]) or os.mkdir(
                    pyQMD_PATH["trash"])
                Out.success('trash bin has been cleaned out')
        if '-l' in args:
            for trashed in os.listdir(pyQMD_PATH["trash"]):
                Out.out(f'{trashed}')
        if '-r' in args:
            name = ''
            tmp = []
            ...
            name = name[1:]
            if name.strip():
                shutil.move(pathlib.Path(f"{pyQMD_PATH['trash']}/{name}"), os.getcwd())
                return
...
[ you can also run: [ qfetch and/or netscan ] for more information ]''')
    ...
    def help(cls, *args):
        if '-e' in args:
            with open(pathlib.Path(os.path.realpath(f'{ROOT}/commands/pyQMDexamples.man')), 'r') as f:
                Out.out(f.read())
            return
        with open(pathlib.Path(os.path.realpath(f'{ROOT}/commands/pyQMDline.man')), 'r') as f:
            Out.out(f.read())
...
class Requests:
    ...
    def ping(cls, *args):
        for _ in range(10):
            try:
                r = requests.get(args[0], timeout=30)
                Out.out(f'{r.status_code} - {r.reason}')
            except (requests.exceptions.BaseHTTPError, requests.exceptions.InvalidURL, requests.exceptions.HTTPError,
                    requests.exceptions.RequestException):
                Out.err(f'[ HTTP-ERROR ] couldn\'t connect to {args[0]}')
...
class Process:
    ...
    def processes(cls, *args):
        ...
        Out.success(f'TOTAL PROCESSES RUNNING: {count}')
    ...
    def find_by_name(cls, *args):
        ...
        name = name[1:].strip()
        for proc in psutil.process_iter():
            if name in proc.name() or proc.name() in name:
                valid.append((proc.name(), proc.pid))
                count += 1
        if valid:
            Out.out(tabulate(valid, headers=['NAME', 'PID']))
            Out.success(f'PROCESSES RUNNING WITH "{name}": {count}')
            return
        Out.err(f'[ PRC ] NO PROCESSES RUNNING WITH "{name}"')
    ...
    def find_by_pid(cls, *args):
        ...
        for piece in args:
            prc_pid += f' {piece}'
        prc_pid = int(prc_pid[1:])
        for proc in psutil.process_iter():
            if proc.pid == prc_pid:
                Out.out(tabulate([(proc.name(), proc.pid)], headers=['NAME', 'PID']))
                return
        Out.err(f'[ PRC ] NO PROCESSES RUNNING WITH THE PID OF {prc_pid}')

    # ??happy?? 500 lines of code; this used to be the 500th line
    ...
    def kill_pid(cls, *args):
        try:
            ...
            Out.err(f"[ PRC ] can't kill the process {args[0]}, {e}")
            return
...
class Random:
    ...
    @classmethod
    def bug_report(cls, *args):
        webbrowser.open('https://github.com/B00bleaTea/pyQMDline/issues')


# i'm out of ideas and i wish i was unalive
# line: 563
...
class CommandUtils:
    ...
    def back(cls, *args):
        handlers = eval(open(pathlib.Path(f'{ROOT}/commands/handlers.hand'), 'r').read())
        with open(pathlib.Path(f'{ROOT}/.pyQMD_history'), 'r') as f:
            qmds = [line[:-1] for line in f.readlines()]
            qmds.reverse()
            qmds = qmds[1].split(' ')
        if '-R' in args:
            handlers[qmds[0]](*qmds[1:])
            return
        handlers[qmds[0]](*qmds[1:]) if qmds[0] != '!!' else Out.err('[ RECURSION ] command tried calling itself')


class History:
    @classmethod
    def clear(cls):
        open(pathlib.Path(f'{ROOT}/.pyQMD_history'), 'w').close()

    @classmethod
    def history(cls, *args):
        if '-C' in args:
            try:
                History.clear()
                Out.success('your command history has been cleared')
            except Exception as e:
                Out.err(f'unable to clear your history - {e}')
            return
        with open(pathlib.Path(f'{ROOT}/.pyQMD_history'), 'r') as f:
            for h_item in f.readlines():
                Out.out(h_item.strip('\n'))
